package nl.hoozemans.esb.utilities.webservices;

import java.util.ArrayList;
import java.util.List;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

public class Binding extends NamespaceBase
{
	public Binding( Base parent, String source )
	throws Exception
	{
		super(parent, source, null);
		parse();
	}
	
	public boolean parse()
	throws Exception
	{
		return super.parse();
	}

	public boolean replaceNamespacePrefix( String oldPrefix, String newPrefix )
	throws Exception
	{
		if( getNamespaceByPrefix(oldPrefix) != null )
			return false;
		
		return replaceNamespacePrefixInSource(oldPrefix, newPrefix);
	}	

	public static List<Binding> extract(Base parent, String from)
	throws Exception
	{
		List<Binding> result = new ArrayList<Binding>();
		
		String pattern = "<" + PATTERN_ANYNSPREFIX + "binding(.*?)>(.*?)</" + PATTERN_ANYNSPREFIX + "binding>";
		Pattern regex = Pattern.compile(pattern, Pattern.CASE_INSENSITIVE | Pattern.DOTALL );
		Matcher matcher = regex.matcher(from);
		
		while( matcher.find() )
		{
			Binding binding = new Binding(parent, matcher.group(0));
			result.add(binding);
		}
		
		return result;
	}

}
